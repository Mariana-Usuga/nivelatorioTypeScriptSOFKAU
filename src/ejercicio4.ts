...
function compararPromedio(numeroPromedio: number, numeroAComparar:number): string{
  if(numeroPromedio > numeroAComparar) return `mayor`
  if(numeroPromedio < numeroAComparar) return `menor`
  return `igual`
}
const promed = promedio(1,4,5,6)
...
function promedioMayorMenor(numeroAComparar:number, ...numeros: number[]): string{
  const p = promedio(...numeros)
  const comparacion = compararPromedio(p, numeroAComparar)
  if(p > numeroAComparar) return `El numero promedio es ${p} y es ${comparacion} a ${numeroAComparar}`
  if(p < numeroAComparar) return `El numero promedio es ${p} y es ${comparacion} a ${numeroAComparar}`
